...
EXP_ID = 'informed' # the ID of this experiment (used to create log names)
...
def fitness(ind, weights):
    ...
    normalized_bw = np.array(bw) / sum(bw)
    entropy = - sum([p * np.log2(p) for p in normalized_bw if p > 0])
    return utils.FitObjPair(fitness=entropy, objective=max(bw) - min(bw))
...
def tournament_selection(pop, fits, k):
    ...
    selected = []
    for _ in range(k):
        idxs = np.random.choice(len(pop), 2)
        best_fit = np.argmax([fits[i] for i in idxs])

        if random.random() < p_bad:
            best_fit = 1 - best_fit

        selected.append(pop[idxs[best_fit]])
    return selected
...
def informed_mutate(p, prob, upper):
    from copy import deepcopy
    result = deepcopy(p)

    bw = bin_weights(weights, p)

    heavy_bin = max(range(K), key=lambda i: bw[i])
    light_bin = min(range(K), key=lambda i: bw[i])
    
    one_heavy = np.random.choice([i for i, b in enumerate(p) if b == heavy_bin])
    result[one_heavy] = light_bin
...
if __name__ == '__main__':
    ...
    mut = functools.partial(mutation, mut_prob=MUT_PROB, 
                            mutate=functools.partial(informed_mutate, prob=MUT_FLIP_PROB, upper=K))
    ...
    utils.plot_experiments('partition', ['default', 'tournament', 'entropy', 'tweaked', 'informed'], 
                           rename_dict={
        'default': 'Default settings', 
        'tournament': 'Default + tournament selection',
        'entropy': 'Default + tournament selection + entropy fitness',
        'tweaked': 'Default + tournament selection + entropy fitness + tweaked hyperparameters',
        'informed': 'Default + tournament selection + entropy fitness + tweaked hyperparameters + informed mutation',})
